...
Feature: View AI Analysis Reports
  ...
  Scenario: Failed reports show technical details when Moodle debug display is enabled
    Given the following config values are set as admin:
      | debug        | 32767 |
      | debugdisplay | 1     |
    And I am on the "Course 1" "Course" page logged in as "teacher1"
    When I navigate to "Reports > AI Conversation Analysis" in current page administration
    Then I should see "Sensitive connector details" in the "Failed Analysis" "table_row"
    When I view the AI analysis report "Failed Analysis"
    Then I should see "Sensitive connector details" in the ".alert-danger" "css_element"
